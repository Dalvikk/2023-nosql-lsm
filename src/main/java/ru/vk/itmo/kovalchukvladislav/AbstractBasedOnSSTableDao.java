...
public abstract class AbstractBasedOnSSTableDao<D, E extends Entry<D>> extends AbstractInMemoryDao<D, E> {
    ...
    private void readFileAndMapToSegment(String filenamePrefix, int index,
        ...
        dstChannel[index] = FileChannel.open(path, StandardOpenOption.READ);
        dstSegment[index] = dstChannel[index].map(FileChannel.MapMode.READ_ONLY, 0, Files.size(path), arena);
    }
    ...
    public Iterator<E> get(D from, D to) {
        ...
        for (int i = 0; i < storagesCount; i++) {
            storageIterators.add(new StorageIterator(dbMappedSegments[i], offsetMappedSegments[i], from, to));
        }
